...
describe("BCS Deserializer", () => {
  ...
  it("deserializes dynamic length bytes", () => {
    ...
    expect(deserializer.deserializeBytes()).toEqual(
  ...
  it("deserializes fixed length bytes", () => {
    ...
    expect(deserializer.deserializeFixedBytes(5)).toEqual(
      new Uint8Array([0x41, 0x70, 0x74, 0x6f, 0x73]),
    );
  });
  ...
  it("deserializes a uint64", () => {
    ...
    expect(deserializer.deserializeU64()).toEqual(
      BigInt("18446744073709551615"),
    );
    deserializer = new Deserializer(
      new Uint8Array([0x00, 0xef, 0xcd, 0xab, 0x78, 0x56, 0x34, 0x12]),
    );
    expect(deserializer.deserializeU64()).toEqual(
      BigInt("1311768467750121216"),
    );
  });
  ...
  it("deserializes a uint128", () => {
    ...
    expect(deserializer.deserializeU128()).toEqual(
      BigInt("340282366920938463463374607431768211455"),
    );
    deserializer = new Deserializer(
      new Uint8Array([
        0x00, 0xef, 0xcd, 0xab, 0x78, 0x56, 0x34, 0x12, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00,
      ]),
    );
    expect(deserializer.deserializeU128()).toEqual(
  ...
  it("deserializes a uleb128", () => {
    let deserializer = new Deserializer(
      new Uint8Array([0xcd, 0xea, 0xec, 0x31]),
    );
