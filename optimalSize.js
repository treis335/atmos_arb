// optimalSize.js — Ternary search para tamanho óptimo por rota
// v2: simulateRoute usa callView com retry; fallback AMM robusto
...
async function simulateRoute(route, amountIn, poolsMap) {
  ...
  for (const hop of route) {
    ...
    let amountOutRaw = await simulateSwap(hop.pool, hop.from, hop.to, amountInRaw, pool.poolType);
    ...
    if (!amountOutRaw || amountOutRaw <= 0) {
      ...
      amountOutRaw = Math.floor((aIn * rOut * (10 ** decOut)) / (rIn + aIn));
    }

    if (!amountOutRaw || amountOutRaw <= 0) return null;

    hopOutputs.push({ amountInRaw, expectedOutRaw: amountOutRaw });
    amount = amountOutRaw / (10 ** decOut);
  }

  return { finalAmount: amount, hopOutputs };
}

async function findOptimalAmount(route, poolsMap, minIn = 5, maxIn = 5000, iterations = 18) {
