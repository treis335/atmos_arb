// src/config/tokens.js — registo de tokens conhecidos
// FA tokens: endereço nu (sem '::')
// Coin tokens: endereço::module::CoinType
...
const TOKENS = {
  ...
  // FA tokens — endereço nu, resolvidos em runtime via get_fa_details_multi
...
for (const [key, info] of Object.entries(TOKENS)) {
  const t = info.type ?? key; // FA tokens usam a key como type
  TYPE_TO_KEY[t] = key;
}

function getByType(type)   { return TOKENS[TYPE_TO_KEY[type]] ?? TOKENS[type] ?? null; }
function getSymbol(type)   {
  const t = TOKENS[TYPE_TO_KEY[type]] ?? TOKENS[type];
  if (t?.symbol) return t.symbol;
  return type.includes('::') ? type.split('::').pop() : type.slice(0,6)+'..'+type.slice(-4);
}
...
function registerRuntime(type, symbol, decimals = 8) {
  ...
  TOKENS[type] = { symbol, decimals };
  TYPE_TO_KEY[type] = type;
}
